...
const authenticationServicePort = 3002;
// const authenticationServiceHost = 'http://authentication-service';
const authenticationServiceHost = 'http://localhost';
// const authenticationServiceHost = 'http://192.168.1.73:3002';
	...
	const connect = async retries => {
		try {
			return await mongoose.connect(
					`mongodb://${ dbUsername }:${ dbPassword }@${ dbHost }:${ dbPort }/${ database }?authSource=admin`,
	...
	.then( dbresult => {
		...
		// db.once( 'open', function () {
			...
			app.post( '/register', async ( req, res ) => {
				...
				const options = {
					method: 'POST',
					uri: `${ authenticationServiceHost }:${ authenticationServicePort }/register`,
					body: { email, password },
					json: true
				};
			...
			app.post( '/authorize', async ( req, res ) => {
				...
				try {
					...
					console.log( 'error', error.message );
					return responseError( res, 'Could not fetch user data right now, please try later', 500 );
				}

				const options = {
					method: 'POST',
					uri: `${ authenticationServiceHost }:${ authenticationServicePort }/authenticate`,
					body: { email, password },
					json: true
				};
